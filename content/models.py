...
class Subject(TimestampedModel):
	...
	# Store objectives as a single comma-separated string, but expose
	# them as a list of strings via the serializer layer.
...
class Story(TimestampedModel):
	...
	characters = models.JSONField(default=list, blank=True)  # [{name, description, ...}]
	vocabulary = models.JSONField(default=list, blank=True)  # [{word, definition}]
	moral = models.CharField(max_length=255, blank=True, default="")

	# Cover image generated by AI (store prompt + URL)
